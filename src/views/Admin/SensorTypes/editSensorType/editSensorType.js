...
export default class editSensorType extends React.Component {
    state = {
        form: {
            type: "",
            unit: "",
    ...
    handleSubmit = async (e) => {
        ...
        if (isValid) {
            ...
            try {
                ...
                if (response.status === 200) {
                    this.setState({ snackMessage: { message: "Tipo de editado correctamente!", success: true }, submitted: true, })
                }
                else {
                    this.setState({
                        snackMessage: { message: "No se pudo realizar la edición", success: false }, openSnackbar: true
                    })
                }
            } catch (e) {
                this.setState({
